...
import { fetchMeEffect } from '../../features/auth/auth.effect';
...
function LimitBanner({ message, onClose }: { message: string; onClose: () => void }) {
  return (
    <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
      <p className="text-blue-800">{message}</p>
      <button onClick={onClose} className="mt-2 text-sm text-blue-600 hover:text-blue-800">
        Close
      </button>
    </div>
  );
...
function NavBar({ user, onLogout }: { user: { name: string; isPremium: boolean }; onLogout: () => void }) {
  return (
    <nav className="bg-white shadow-sm border-b">
      <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={onLogout} className="px-4 py-2 text-sm text-red-600 hover:text-red-700">
            Log Out
          </button>
        </div>
      </div>
...
export default function DashboardPage() {
  const { user, logout, isLoading, refreshUser, login } = useAuth();
  ...
  useEffect(() => {
    ...
    void Effect.runPromise(Effect.either(fetchMeEffect())).then((either) => {
      if (Either.isRight(either)) {
        const { user: freshUser, token: freshToken } = either.right;
        if (freshUser.isPremium !== user.isPremium) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {limitMessage && <LimitBanner message={limitMessage} onClose={() => { setLimitMessage(null); }} />}
